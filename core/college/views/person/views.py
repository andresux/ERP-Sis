...
class PersonListView(AccessModuleMixin, PermissionModuleMixin, ListView):
    model = Person
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = 'Listado de Profesores y Estudiantes'
        return context


class PersonCreateView(AccessModuleMixin, PermissionModuleMixin, CreateView):
    model = Person
        ...
        try:
            if action == 'add':
                with transaction.atomic():
                    ...
                    pers.type = request.POST['type']
                    if pers.type == 'docente':
                        pers.prof_id = request.POST['profession']
                        if 'cvitae' in request.FILES:
                            pers.cvitae = request.FILES['cvitae']
                    pers.save()

                    pk = STUDENT if pers.type == 'estudiante' else TEACHER
...
class PersonUpdateView(AccessModuleMixin, PermissionModuleMixin, UpdateView):
    model = Person
    template_name = 'person/create.html'
...
class PersonDeleteView(AccessModuleMixin, PermissionModuleMixin, DeleteView):
    model = Person
