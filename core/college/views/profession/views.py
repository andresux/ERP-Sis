...
from core.college.forms import ProfessionForm, Profession
from core.security.mixins import AccessModuleMixin, PermissionModuleMixin


class ProfessionListView(AccessModuleMixin, PermissionModuleMixin, ListView):
    model = Profession
...
class ProfessionCreateView(AccessModuleMixin, PermissionModuleMixin, CreateView):
    model = Profession
...
class ProfessionUpdateView(AccessModuleMixin, PermissionModuleMixin, UpdateView):
    model = Profession
    template_name = 'profession/create.html'
    form_class = ProfessionForm
    success_url = reverse_lazy('profession_list')
    ...
    def validate_data(self):
        ...
        try:
            ...
            obj = self.request.POST['obj'].strip()
            if type == 'name':
                if Profession.objects.filter(name__iexact=obj).exclude(pk=id):
                    data['valid'] = False
        except:
            pass
        return JsonResponse(data)
...
class ProfessionDeleteView(AccessModuleMixin, PermissionModuleMixin, DeleteView):
    model = Profession
    ...
    def post(self, request, *args, **kwargs):
        data = {}
        try:
            Profession.objects.get(pk=self.get_object().id).delete()
        except Exception as e:
            data['error'] = str(e)
        return HttpResponse(json.dumps(data), content_type='application/json')
