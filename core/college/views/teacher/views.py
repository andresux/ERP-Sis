...
class TeacherListView(AccessModuleMixin, PermissionModuleMixin, ListView):
    model = Person
    template_name = 'teacher/list.html'
    permission_required = 'view_person'

    def get_queryset(self):
        return Person.objects.filter(type='docente')
...
class TeacherCreateView(AccessModuleMixin, PermissionModuleMixin, CreateView):
    model = Person
        ...
        try:
            if action == 'add':
                with transaction.atomic():
                    ...
                    pers.type = 'docente'
                    pers.prof_id = request.POST['profession']
                    if 'cvitae' in request.FILES:
                        pers.cvitae = request.FILES['cvitae']
                    pers.save()

                    group = Group.objects.get(pk=TEACHER)
...
class TeacherUpdateView(AccessModuleMixin, PermissionModuleMixin, UpdateView):
    model = Person
    template_name = 'teacher/create.html'
    ...
    def get_form(self, form_class=None):
        form = PersonChangeForm(instance=self.object.user, initial={
            ...
            'profession': self.object.prof_id,
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data()
        context['list_url'] = self.success_url
        context['title'] = 'Edición de un Docente'
        context['action'] = 'edit'
        return context


class TeacherDeleteView(AccessModuleMixin, PermissionModuleMixin, DeleteView):
    model = Person
